// Script to generate 20,000+ Android devices for performance testing
...
function generateDeviceId(manufacturer, index) {
...
function generateModelName(manufacturer, index) {
  const phoneNames = [
...
function generateDevices(count = 20000) {
  const devices = [];
  ...
  for (let i = 0; i < count; i++) {
    ...
    if ((i + 1) % 1000 === 0) {
      console.log(`Generated ${i + 1} devices...`);
    }
  }
...
// Generate 20,000 devices
console.log('Generating 20,000 Android devices for performance testing...');
const devices = generateDevices(20000);

// Write to JSON file
const fs = require('fs');
const outputPath = './android-devices-20k.json';

console.log('Writing to file...');
fs.writeFileSync(outputPath, JSON.stringify(devices, null, 2));

console.log(`✅ Successfully generated ${devices.length} devices and saved to ${outputPath}`);
